    ...
    def partitions(self) -> List[CBORInputPartition]:
        ...
        if path.startswith("s3://"):
            try:
                ...
                raise ImportError("Boto3 is required to read this file. pip install pyspark-cbor[aws]")
            # Handle S3 path
            s3 = boto3.client("s3")
            ...
            if prefix.endswith("/"):
                ...
                file_names = [f"s3://{bucket}/{item['Key']}" for item in response.get("Contents", []) if
            ...
            try:
                ...
                raise ImportError("google-cloud-storage is required to read this file. pip install pyspark-cbor[gcp]")
            # Handle GCS path
            ...
            if prefix.endswith("/"):
                ...
                file_names = [f"gs://{parsed_url.netloc}/{blob.name}" for blob in blobs if not blob.name.endswith("/")]
            else:
                file_names = [path]
            ...
            try:
                ...
                raise ImportError("azure-storage-blob is required to read this file. pip install pyspark-cbor[azure]")
            ...
            if prefix.endswith("/"):
                ...
                file_names = [f"https://{account_name}.blob.core.windows.net/{container_name}/{blob.name}" for blob in
                              blobs if not blob.name.endswith("/")]
            else:
                file_names = [path]
        ...
        return [CBORInputPartition(file_name=file_name.strip()) for file_name in file_names]
    ...
    def read(self, partition: CBORInputPartition) -> Iterator[Tuple]:
        ...
        for record in records:
            if not isinstance(record, dict):
                raise ValueError(f"Expected record to be a dict, but got {type(record).__name__}: {record}")
